                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" id="content" class="form-control" required>{{ old('content', $product->content) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="material">Materyal</label>
                            <textarea name="material" id="material" class="form-control" required>{{ old('material', $product->material) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="dimensions">Boyutlar</label>
                            <textarea name="dimensions" id="dimensions" class="form-control" required>{{ old('dimensions', $product->dimensions) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="color_options">Renk Seçenekleri</label>
                            <textarea name="color_options" id="color_options" class="form-control" required>{{ old('color_options', $product->color_options) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="guarantee">Garanti</label>
                            <textarea name="guarantee" id="guarantee" class="form-control" required>{{ old('guarantee', $product->guarantee) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="montaj">Montaj</label>
                            <textarea name="montaj" id="montaj" class="form-control" required>{{ old('montaj', $product->montaj) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="tags">Etiketler</label>
                            <input type="text" name="tags" id="tags" class="form-control" value="{{ old('tags', $product->tags) }}" placeholder="Etiketleri virgülle ayırın">
                        ...
                        <div class="form-group">
                            ...
                            @if($product->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->title }}" class="img-thumbnail" style="max-height: 200px;">
                                </div>
                            @endif
